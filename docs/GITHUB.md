...
在 GitHub 的 Releases 页面创建 `v0.1.0` 发布，并上传两个文件：

```text
vpngate-manager-0.1.0.tar.gz
vpngate-manager-0.1.0.tar.gz.sha256
```
...
```bash
curl --proto '=https' --tlsv1.2 -fsSL https://raw.githubusercontent.com/OWNER/REPOSITORY/v0.1.0/scripts/install-from-github.sh | sudo bash -s -- --repo OWNER/REPOSITORY --version 0.1.0
```
